...
level: 2
---
> **Claude-safe portable degraded skill:** `project-session-manager` is bundled for prompt guidance only in this no-plugin `.claude` bundle. Do not install OMC, fetch from the internet, register hooks/MCP, launch tmux/runtime services, or write to global `~/.claude` paths unless the user explicitly authorizes it and `claude-safe` permits it. Prefer local repository evidence and the root safe-mode overlay.
